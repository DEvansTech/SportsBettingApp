...
const NCAABWatch: React.FC<Props> = ({
  data,
  saveSelection,
  ...
  const checkPeriod = () => {
    if (!checkStatus() && data.period) {
      if (Number(data.period) > 2) return 'Overtime';
      return `${ordinalSuffixOf(data.period)} Half`;
    }
  };
  ...
  const checkStatus = () => {
    if (
      ...
      (Number(data.period) === 1 && data.clock === '00:00')
    ) {
      return 'Halftime';
  ...
  const checkClock = () => {
    if (!checkStatus()) {
      if (Number(data.period) === 1 && data.clock === '00:00') return;
      if (
  ...
  const saveToSelection = () => {
    if (user && saveSelection) {
      saveSelection(data.gameID, 'ncaab');
      Vibration.vibrate();
    } else {
    ...
    <View style={[styles.container, lastGame && styles.noBorder]}>
      <View style={styles.gameView}>
        ...
        <View style={styles.scoreView}>
          ...
          {/* {(data.status === 'inprogress' || data.status === 'halftime') &&
            data.away_points &&
            data.home_points && <BasketBallUpdate disabled={true} />} */}
        </View>
        <View style={styles.teamStatusView}>
          <LoadingImage
