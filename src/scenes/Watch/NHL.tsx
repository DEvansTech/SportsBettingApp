...
import { SubHeader, NHLWatch, Loading } from '@Components';
...
const NHL: React.FC<Props> = ({ selectedDate, sportName }) => {
  ...
  const saveSelections = (gameIDs: number[]) => {
    if (user) {
      const docRef = firestore().collection('nhlSelections').doc(user.uid);
  ...
  const renderMyTeam = (match: GameDataType, index: number) => {
    return (
      <NHLWatch data={match} lastGame={teamsGamedata.length === index + 1} />
    );
  };
